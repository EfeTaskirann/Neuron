/* Inline icon system for the Neuron prototype.
   Lucide-style: 24×24 viewBox, stroke 1.75, round caps + joins, currentColor.
   Attaches NIcon, NodeGlyph, Brandmark to window so shell.jsx, routes.jsx,
   canvas.jsx, inspector.jsx, terminal.jsx can use them directly. */
/* global React */
...
/* ---------- NIcon ---------- */
function NIcon({ name, size = 16, stroke = 1.75, color = "currentColor", style, className = "" }) {
...
/* ---------- NodeGlyph ----------
   kind ∈ { llm, tool, logic, human, mcp }
   Returns a rounded badge with a token-bound color tint and a kind-specific glyph.
   Used in: routes.jsx (agent-avatar), routes.jsx (server-icon for MCP cards),
            canvas.jsx (node-card head), inspector.jsx (span row icon). */
const KIND_TINT = {
  llm:   { color: "var(--neuron-violet-300)",  bg: "color-mix(in oklch, var(--neuron-violet-500) 18%, transparent)", border: "color-mix(in oklch, var(--neuron-violet-400) 55%, var(--border))" },
  tool:  { color: "var(--neuron-sky-300)",     bg: "color-mix(in oklch, var(--neuron-sky-500) 18%, transparent)",    border: "color-mix(in oklch, var(--neuron-sky-400) 55%, var(--border))"    },
  logic: { color: "var(--neuron-slate-300)",   bg: "color-mix(in oklch, var(--neuron-slate-500) 22%, transparent)",  border: "color-mix(in oklch, var(--neuron-slate-400) 55%, var(--border))"  },
  human: { color: "var(--neuron-amber-300)",   bg: "color-mix(in oklch, var(--neuron-amber-500) 16%, transparent)",  border: "color-mix(in oklch, var(--neuron-amber-400) 55%, var(--border))"  },
  mcp:   { color: "var(--neuron-violet-200)",  bg: "linear-gradient(135deg, var(--neuron-midnight-800), var(--neuron-midnight-950))", border: "color-mix(in oklch, var(--neuron-violet-500) 35%, var(--border))" },
};
...
const KIND_GLYPH = {
  ...
  tool: (
    <>
      <path d="M14.5 6.5L18 3l3 3-3.5 3.5h-2l-8.5 8.5L4 21l-3-3 5-5 2 2 8.5-8.5z" />
    </>
  ),
  logic: (
    <>
...
function NodeGlyph({ kind = "llm", size = 24 }) {
  const tint = KIND_TINT[kind] || KIND_TINT.llm;
  const glyph = KIND_GLYPH[kind] || KIND_GLYPH.llm;
...
/* ---------- Brandmark ----------
   Inline replica of assets/brandmark.svg.
   Three terminals on a violet gradient over midnight-900. */
