//! Acceptance + input-validation coverage for the `swarm:*` command
//! surface. Extracted from `swarm.rs` (T3-01 follow-up, 2026-06-03) so
//! the module root carries only the submodule wiring + re-exports while
//! this file holds the full `mod tests` block intact.
//!
//! The tests stayed in one module (rather than fanning out per
//! submodule) because they share scaffolding helpers
//! (`mock_app_with_w5_state`, `mock_app_with_brain_state`,
//! `BrainScriptedInvoker`, `seed_swarm_job_row`) and reach every command
//! through the `commands::swarm` re-exports, so `use super::*;` resolves
//! the same as the pre-split single-file version.
